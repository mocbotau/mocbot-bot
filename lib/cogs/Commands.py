...
class Commands(commands.Cog):
    ...
    @app_commands.command(
        name="announce",
        description="Announces a message to a given audience and channel",
    )
    ...
    async def announce(
        ...
        audience: Literal["none", "here", "everyone"],
        ...
        if older_messages:
            ...
            await interaction.followup.send(
                embed=self.bot.create_embed(
                    "MOCBOT PURGE",
                    f"**{len(older_messages)}** message(s) over 14 days old were found. This will take roughly"
                    f" {await convert(0.75 * len(older_messages))} to delete. Are you sure you'd like to continue?",
                    0xFFA500,
                ),
                ephemeral=True,
                view=view,
            ...
            if view.confirmed:
                ...
                await interaction.channel.delete_messages(
                    recent_messages,
                    reason=f"{interaction.user} purged messages",
                )
                await interaction.followup.send(
                    f"**{len(to_purge)}** messages deleted. Channel purge complete.",
                    ephemeral=True,
                )
        else:
    ...
    async def setup(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=self.bot.create_embed(
                "MOCBOT SETUP",
                f"To ensure full functionality of {self.bot.user.mention}, you must setup the bot to accomodate your "
                "server.",
                None,
            ),
            ephemeral=True,
    ...
    async def invite(self, interaction: discord.Interaction):
        ...
        view.add_item(
            discord.ui.Button(
                label="Invite MOCBOT",
                style=discord.ButtonStyle.link,
                url="https://discord.com/api/oauth2/authorize?client_id=417962459811414027&permissions=8&scope=bot"
        ...
        await interaction.response.send_message(
            embed=self.bot.create_embed(
                "MOCBOT SETUP",
                "Use the button below to invite MOCBOT into your own server!",
                None,
            ),
            view=view,
        )
    ...
    async def contact(self, interaction: discord.Interaction):
        ...
        view.add_item(
            discord.ui.Button(
                label="Contact us",
                style=discord.ButtonStyle.link,
                url="https://masterofcubesau.com/contact",
            )
        )
        await interaction.response.send_message(
            embed=self.bot.create_embed(
                "MOCBOT CONTACT",
                "Use the button below to contact the team.",
                None,
            ),
            view=view,
        )
    ...
    async def help(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=self.bot.create_embed(
                "MOCBOT HELP",
                "Use the button below to get help using MOCBOT.",
                None,
            ),
            view=view,
        )
    ...
    async def dashboard(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=self.bot.create_embed(
                "MOCBOT HELP",
                "Use the button below to access the MOCBOT dashboard.",
                None,
            ),
            view=view,
        )
    ...
    async def account(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=self.bot.create_embed(
                "MOCBOT HELP",
                "Use the button below to access your MOCBOT account.",
                None,
            ),
            view=view,
        )
    ...
    async def info(self, interaction: discord.Interaction, member: Optional[discord.Member]):
        ...
        if isinstance(target, discord.Member):
            ...
            **{target}** was created at `{target.created_at.strftime("%I:%M%p, %d/%m/%Y %Z")}`
             and is owned by {target.owner.mention}
            """
